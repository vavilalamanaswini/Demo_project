...
test.describe("LogIn", () => {
        ...
        test.skip("Invalid phone number @17", async ({ page }) => {
        ...
        test.skip("Wrong OTP @18", async ({ page }) => {
                const homePage = new HomePage(page);
                await page.goto('/');
                await homePage.openLogin();
        ...
        test.skip("Change Language and verify @19", async ({ page }) => {
                const homePage = new HomePage(page);
                await page.goto('/');
                await homePage.changeLanguage(language.name, language.displayText);
        });

        test.skip("Change Location and verify @20", async ({ page }) => {
                const homePage = new HomePage(page);
                await page.goto('/');
